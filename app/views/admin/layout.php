    ...
    <div class="flex h-screen">
        ...
        <aside class="w-64 bg-primary-900 text-white flex-shrink-0">
            ...
            <nav class="mt-6">
                ...
                <a href="<?= ADMIN_URL ?>?action=programs" class="block px-6 py-3 hover:bg-secondary-700 transition-colors <?= in_array($_GET['action'] ?? '', ['programs', 'program_create', 'program_edit', 'program_media_upload', 'program_video_upload', 'program_video_url_save', 'program_media_reorder', 'program_media_delete', 'program_media_set_cover', 'program_media_caption_save'], true) ? 'bg-secondary-700 border-l-4 border-accent-400' : '' ?>">
                    <span class="flex items-center">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
